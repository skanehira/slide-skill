...
**方法A：Claude Codeに頼む（おすすめ）**

Claude Codeのチャットに以下のように話しかけるだけでインストールできます。

> 「このリポジトリのスキルをインストールして: https://github.com/sho-ai-magic/slide.md」

Claude Codeがリポジトリを読み込み、3つのスキルを `~/.claude/skills/` に自動でセットアップします。
...
各フォルダに `SLIDE.md`（デザイン定義）と `sample.html`（確認用6ページHTMLスライド）が含まれています。

**サンプルをすぐに使うには**

デザインシステムとスライドパターンをまとめてプロジェクトに導入するには、Claude Codeに以下のように頼むだけです。

> 「`https://github.com/sho-ai-magic/slide.md` にあるすべてのデザインシステムサンプルとスライドパターンを今のプロジェクトフォルダにコピーして」

Claude Codeがリポジトリを読み込み、`examples/design-systems/` 配下の4種類のデザインシステムと `docs/patterns/` 配下の99種類のスライドパターンをプロジェクトフォルダへコピーします。コピー後はすぐに `slide-deck-builder` スキルが利用できる状態になります。

特定のデザインシステムだけ使いたい場合は [ギャラリー](https://sho-ai-magic.github.io/slide.md/) でカードをクリックし、「SLIDE.md」タブからファイル内容をコピーして、プロジェクトフォルダ内の `SLIDE-md-{name}/SLIDE.md` として保存してもOKです。
