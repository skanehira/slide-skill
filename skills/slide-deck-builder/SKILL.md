...
カレントディレクトリに無い場合、このSKILL.mdから見て `../../docs/SLIDE-md/SLIDE-md-*/SLIDE.md`（プラグイン同梱の9種類のサンプルデザインシステム）を検索する。

**見つかった場合：**

各ファイルのOverview（マッチするシーン）を読み取って以下の形式で提示する：

> 「カレントディレクトリにはデザインシステムが見つかりませんでしたが、プラグイン同梱のサンプルデザインシステムが9種類あります。どれを使いますか？番号で教えてください。
>
> 1. `SLIDE-md-{source1}` — [SLIDE.mdのOverview「マッチするシーン」の内容]
> 2. `SLIDE-md-{source2}` — [同上]
> ...」

選ばれたサンプルの `SLIDE-md-{name}` フォルダ（`SLIDE.md` と `sample.html`）をカレントディレクトリの `SLIDE-md/SLIDE-md-{name}/` にコピーしてから読み込む（以後の再利用・カスタマイズのためプロジェクトに永続化する）。

**プラグイン同梱サンプルにも見つからない場合（想定外のケース）：**

> 「デザインシステム（SLIDE.md）が見つかりませんでした。`slide-md-creator` スキルを実行してSLIDE.mdを作成しますか？」

承認を得たらその場で `slide-md-creator` スキルを呼び出し、SLIDE.mdの生成が完了したら自動でSTEP 3の検索に戻る（ユーザーに別スキルを手動で起動し直させない）。拒否された場合はスキルを終了する。
...
パターン定義（`SLIDE-PATTERN/`）は読み取り専用の共有資産のため、プロジェクトへのコピーはせず、以下の優先順位でインデックスファイルを直接読み込む。

1. カレントディレクトリの `SLIDE-PATTERN/SLIDE-PATTERN-INDEX.md`（ユーザーが独自パターンを追加している場合はこちらを優先）
2. 無ければ、このSKILL.mdから見て `../../docs/SLIDE-PATTERN/SLIDE-PATTERN-INDEX.md`（プラグイン同梱の99パターン）を直接読み込む

以降、どちらのパスから読み込んだかを記憶しておき、STEP 6でパターン定義本体を参照する際も同じ場所（`SLIDE-PATTERN/` または `../../docs/SLIDE-PATTERN/`）を使う。

### どちらにも見つからない場合（想定外のケース）

> 「`SLIDE-PATTERN-INDEX.md` が見つかりませんでした。`slide-pattern-creator` スキルを実行してスライドパターンを作成しますか？」

承認を得たらその場で `slide-pattern-creator` スキルを呼び出し、パターン生成が完了したら自動でSTEP 5の読み込みに戻る（ユーザーに別スキルを手動で起動し直させない）。拒否された場合はスキルを終了する。
...
3. **Slide Patterns** セクションを書く（STEP 5で使用が確定したパターン名をもとに、STEP 5で読み込んだ場所（カレントの `SLIDE-PATTERN/SLIDE-PATTERN-{name}/SLIDE-PATTERN-{name}.md`、または同梱の `../../docs/SLIDE-PATTERN/SLIDE-PATTERN-{name}/SLIDE-PATTERN-{name}.md`）から各パターン定義を読み込み、全内容をそのままコピーして貼り付ける。使用しないパターンは含めない）
